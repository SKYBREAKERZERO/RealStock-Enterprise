  ...
  validation {
    condition = alltrue(
      [
        ...
        !contains(keys(var.api_environment_variables), reserved_key)
      ]
    )

    error_message = (
      "DATABASE_* connection settings and REDIS_URL are managed by Terraform and must not be overridden through api_environment_variables."
    )
  }
}
...
variable "api_secrets" {
  description = <<-EOT
Additional secret environment-variable mapping consumed by ECS.

Values must be Secrets Manager or SSM Parameter Store ARNs.

Database connection settings are managed automatically by Terraform:
- DATABASE_HOST points to the RDS Proxy endpoint.
- DATABASE_USERNAME is injected from the Aurora master-user secret.
- DATABASE_PASSWORD is injected from the Aurora master-user secret.
- DATABASE_PORT, DATABASE_NAME, and DATABASE_SSLMODE are non-secret
  Terraform-managed runtime settings.
  ...
  validation {
    condition = alltrue(
      [
        ...
        !contains(keys(var.api_secrets), reserved_key)
      ]
    )

    error_message = (
      "DATABASE_URL, DATABASE_USERNAME, DATABASE_PASSWORD, and REDIS_URL are managed by Terraform and must not be overridden through api_secrets."
    )
  }
